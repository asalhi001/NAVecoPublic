...
os.chdir('C:/Users/crybelloceferin/Documents/MATLAB/GUI/Demostrateur/OpenLoop/V41Segmentation2Vehicules')
...
script, departure_adress, arrival_adress, distance_sample = argv

#Adresses
# departure_adress = '93 Bd de la Seine 92000 Nanterre'
# arrival_adress = '127 Av Felix Faure 92000 Nanterre'
# departure_adress = 'Expleo, Avenue des Prés, Montigny-le-Bretonneux'
# arrival_adress = '1098 Avenue Roger Salengro, Chaville'
departure_adress.replace('_',' ')
arrival_adress.replace('_',' ')

#Résolution de la distance entre chaque point pour la récupération de l'altitude
#distance_sample = 1
distance_sample = float(distance_sample)
...
def writeCsv(csvVar,csvName):
    with open('./saved_data/'+csvName+'.csv', 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file,delimiter=';')
        csv_writer.writerows(csvVar)

def loadCsv(csvName):
    csv_list = []
    with open('./saved_data/'+csvName+'.csv', 'r', newline='') as csv_file:
...
for i in range(len(LatSichr)-1):
    ...
    Pente.append((DiffAlt/DistNodes))
    print(' ')
    # print(DiffAlt)
    # print(DistNodes)
...
for i in range(len(LatSichr)):
    DistTemp = route['properties']['segments'][0]['steps'][KeyPointsTemp]['distance']
    DuraTemp = route['properties']['segments'][0]['steps'][KeyPointsTemp]['duration']
    ...
    if(DuraTemp==0):
        ...
        if(DistCum[i]>=DistKeyPoints):
            KeyPointsTemp = KeyPointsTemp + 1
            DistKeyPoints = DistKeyPoints + route['properties']['segments'][0]['steps'][KeyPointsTemp]['distance']
            # DistKeyPointsVect.append(DistKeyPoints)
            # DistTempVect.append(DistTemp)
