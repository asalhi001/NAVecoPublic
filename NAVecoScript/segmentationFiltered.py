...
for i in range(0,len(data)):
    SlopeVFiltre.append(float(data[i][5]))

OrdreFiltre = 100
SlopeVFiltre1 = []
for i in range(0,len(SlopeVFiltre)):
    if len(SlopeVFiltre)>1:
        if i<=OrdreFiltre/2:
            SlopeVFiltre1.append(sum(SlopeVFiltre[:int(OrdreFiltre/2)])/len(SlopeVFiltre[:int(OrdreFiltre/2)]))
        elif i>=(len(SlopeVFiltre)-(OrdreFiltre/2)):
            SlopeVFiltre1.append(sum(SlopeVFiltre[len(SlopeVFiltre)-int(OrdreFiltre/2):])/len(SlopeVFiltre[len(SlopeVFiltre)-int(OrdreFiltre/2):]))
        else:
            SlopeVFiltre1.append(sum(SlopeVFiltre[i-int(OrdreFiltre/2):i+int(OrdreFiltre/2)])/len(SlopeVFiltre[i-int(OrdreFiltre/2):i+int(OrdreFiltre/2)]))

for i in range(0,len(data)):
    data[i][5]=SlopeVFiltre[i]
