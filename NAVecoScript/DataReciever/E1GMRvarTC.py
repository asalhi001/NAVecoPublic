...
for distSample in range(1,10,1):
    for rep in range(10):
        startAll = time.time()
        ...
        print(' ')
        print(' ')
        print('Compilation '+str(rep)+'/10 Lugar: '+str(lieu)+'/10 distSample: '+str(distSample)+'/501')
        print(' ')
        print(' ')
        ...
        ResolutionVector.append(distSample)
...
#Chemin du dossier contenant le programme
# fields = ['ResolutionVector','RouteTimeVector','TotalTimeVector','TotalDistanceVector','TotalNodesVector']
# df = pd.read_csv('C:/Users/crybelloceferin/Documents/MATLAB/Supun/E1Yesid/TempsCalcule/dataTempsCompilRvar.csv', sep=';', skipinitialspace=True, usecols=fields)
# rep = 9
...
# ResolutionVector = df.ResolutionVector

RouteTimeVectorMean = []
TotalTimeVectorMean = []
TotalDistanceVectorMean = []
TotalNodesVectorMean = []
TotalResolVectorMean = []
...
for i in range(int(len(RouteTimeVector)/10)):
    ...
    TotalResolVectorMean.append(sum(ResolutionVector[i*(rep+1):(i+1)*(rep+1)-1])/len(ResolutionVector[i*(rep+1):(i+1)*(rep+1)-1]))

plt.figure(figsize=(10,4))
plt.plot(ResolutionVector,TotalTimeVector,'o')
plt.plot(TotalResolVectorMean,TotalTimeVectorMean,'o')
plt.plot(TotalResolVectorMean,TotalTimeVectorMean)
plt.fill_between(ResolutionVector,TotalTimeVector,alpha=0.1)
...
plt.figure(figsize=(10,4))
plt.plot(ResolutionVector,RouteTimeVector,'o')
plt.plot(TotalResolVectorMean,RouteTimeVectorMean,'o')
plt.plot(TotalResolVectorMean,RouteTimeVectorMean)
plt.fill_between(ResolutionVector,RouteTimeVector,alpha=0.1)
plt.xlabel("Resolution [m]")
...
for l in range(len(RouteTimeVector)):
    DataFinalTemp.append([ResolutionVector[l],RouteTimeVector[l],TotalTimeVector[l],TotalDistanceVector[l],TotalNodesVector[l]])

DataFinalTemp.insert(0,['ResolutionVector','RouteTimeVector','TotalTimeVector','TotalDistanceVector','TotalNodesVector'])
writeCsv(DataFinalTemp,"dataTempsCompilRvarCourt")
